...
contract PriceFeedRouter is MPCManageable {
    ...
    function setDefaultSwapThreshold(uint256[3] calldata threshold)
        external
        onlyManagers
    {
        require(threshold[2] >= threshold[1] && threshold[1] >= threshold[0]);
        _defaultThreshold.low = uint48(threshold[0]);
        _defaultThreshold.mid = uint80(threshold[1]);
        _defaultThreshold.high = uint128(threshold[2]);
    }
